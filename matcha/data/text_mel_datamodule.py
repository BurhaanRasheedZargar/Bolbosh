...
class TextMelDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        # Retry logic for long files
        while True:
            try:
                filepath = self.filepaths_and_text[index][0]
                
                # Heuristic: Skip files > 25 seconds to avoid OOM
                # 22050Hz * 25s = 551250 samples.
                # Hop length 256 -> ~2153 frames.
                
                # Using torchaudio.info to check frames without full load
                info = ta.info(filepath)
                duration_sec = info.num_frames / info.sample_rate
                
                if duration_sec > 25.0:
                    # print(f"Skipping {filepath} (Duration: {duration_sec:.2f}s > 25.0s)")
                    index = random.randint(0, len(self.filepaths_and_text) - 1)
                    continue
                    
                datapoint = self.get_datapoint(self.filepaths_and_text[index])
                return datapoint
            except Exception as e:
                # print(f"Error loading {self.filepaths_and_text[index][0]}: {e}")
                index = random.randint(0, len(self.filepaths_and_text) - 1)
